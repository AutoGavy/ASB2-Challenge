...
function OnTakeDamage_Alive_Any(h_victim, inflictor, h_attacker, weapon, damage, damageType, ammoName) 
{
	...
	{
		if (h_victim != null && h_victim.GetClassname() == "asw_marine" && h_victim != h_attacker)
		{
			if (inflictor != null && inflictor.GetClassname() != "asw_burning" && inflictor.GetClassname() != "asw_sentry_top_machinegun")
			{
				if (weapon != null && weapon.GetClassname() != "asw_sentry_top_cannon")
				{
					...
					{
						damage = damage*3;
						h_attacker.Ignite(60);
					}
					else if (inflictor.GetClassname() == "asw_grenade_vindicator")
						h_attacker.Ignite(60);
					h_attacker.TakeDamage(damage, 64, h_attacker);
					damage = 0;
				}
			}
		}
	...
	else if (h_attacker != null && h_attacker.GetClassname() == "asw_marine")
	{
		if (h_victim != null && h_victim.GetClassname() == "asw_marine")
		{
			...
			SetPoints(attacker, (temp_damage / 20 + 1).tointeger(), 0);
		}
	}
